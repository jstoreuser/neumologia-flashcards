...
# Respostas clínicas estruturadas adicionais para enriquecer o mazo
PREMIUM_DESCRIPTIONS = {
    ...
    43: "<p>En la radiografía o TC de tórax, una cavitación pulmonar suele presentarse con una pared gruesa y un margen interno irregular.</p><p>⚠️ <strong>Diferenciación:</strong> No confundir con el absceso pulmonar, donde se observa un nivel hidroaéreo marcado debido a la presencia de contenido líquido activo (pus, necrosis).</p>",
...
for page_num_str, raw_text in raw_data.items():
    ...
    elif "cuanto tiempo" in raw_prompt.lower() or "estabilidad" in answer.lower() or page_num == 82:
    ...
    if page_num in PREMIUM_DESCRIPTIONS:
        ...
        for block in merged_blocks:
            ...
            # Determinar tipo do bloco e formatar
            if is_modality_label(block):
                ...
                explanation_blocks.append(f'<div class="exp-causes">⚠️ <strong>Causas comunes:</strong> {content}</div>')
